...
export default function ManagerApps() {
  ...
  const createApp = async () => {
    ...
    await supabase.from("activity_logs").insert({ user_id: user.id, action: `Application "${newAppName.trim()}" created` });
      ...
      <AlertDialog open={!!regenerateAppId} onOpenChange={() => setRegenerateAppId(null)}>
        <AlertDialogContent className="bg-card border-border">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will invalidate the current signing secret.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
